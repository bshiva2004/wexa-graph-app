...
// 4. Recommendations by userId
app.get(['/api/recommendations/:userId', '/recommendations/:userId'], async (req, res) => {
...
// 5. Skill Synergy
app.get(['/api/synergy/:userId', '/synergy/:userId'], async (req, res) => {
  const { userId } = req.params;
  let session;
  try {
    session = getSession();
